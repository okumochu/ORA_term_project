...
class EnvState:
    """Environment state for DANIEL environment"""
    
    def __init__(self):
        self.fea_j_tensor = None
        self.op_mask_tensor = None
        self.fea_m_tensor = None
        self.mch_mask_tensor = None
        self.dynamic_pair_mask_tensor = None
        self.comp_idx_tensor = None
        self.candidate_tensor = None
        self.fea_pairs_tensor = None
    
    def update(self, fea_j, op_mask, fea_m, mch_mask, dynamic_pair_mask, comp_idx, candidate, fea_pairs, device=None):
        """Update state tensors"""
        if device is None:
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.fea_j_tensor = torch.from_numpy(fea_j).float().to(device)
        self.op_mask_tensor = torch.from_numpy(op_mask).float().to(device)
        self.fea_m_tensor = torch.from_numpy(fea_m).float().to(device)
        self.mch_mask_tensor = torch.from_numpy(mch_mask).float().to(device)
        self.dynamic_pair_mask_tensor = torch.from_numpy(dynamic_pair_mask).float().to(device)
        self.comp_idx_tensor = torch.from_numpy(comp_idx).float().to(device)
...
class FJSPEnvForVariousOpNums:
    ...
    def __init__(self, n_j, n_m, device=None, reward_type='lb', reward_step=1):
        self.number_of_jobs = n_j
        self.number_of_machines = n_m
        self.old_state = EnvState()
        self.device = device if device is not None else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.reward_type = reward_type  # 'lb' or 'original'
        self.reward_step = reward_step  # Number of steps between rewards (1 = dense, N = sparse)
    ...
    def set_initial_data(self, job_length_list, op_pt_list):
        ...
        # Use filled() to handle masked values safely (same as op_min_pt/op_max_pt)
        self.mch_min_pt = np.max(self.op_pt, axis=1).filled(0)
        self.mch_max_pt = np.max(self.op_pt, axis=1).filled(0)
    ...
    def initial_vars(self):
        ...
        self.previous_makespan = np.zeros(self.number_of_envs)  # For 'original' reward type
        
        # Normalized makespan tracking for 'original' reward type
        self.normalized_makespan = np.full(self.number_of_envs, float("-inf"))
        self.previous_normalized_makespan = np.zeros(self.number_of_envs)
        self.reward_reference_normalized_makespan = np.zeros(self.number_of_envs)
        
        # For multi-step rewards: track reference point from reward_step steps ago
        self.reward_reference_makespan = np.zeros(self.number_of_envs)  # For 'original' reward_step > 1
        self.reward_reference_lb = None  # Will be set after op_ct_lb is initialized
    ...
    def calculate_reward(self):
        """
        Calculate reward based on reward_type and reward_step.
        
        Returns:
            reward: numpy array of shape (number_of_envs,)
        
        Two reward types:
        - 'lb' (lower bound): reward = previous_max_endTime - current_max_endTime
          With gamma=1, cumulative reward = initial_lower_bound - makespan
        - 'original': reward = previous_makespan - current_makespan
          With gamma=1, cumulative reward = -makespan
        
        Reward step (curriculum learning for reward sparsity):
        - reward_step = 1: reward at every step (dense)
        - reward_step = N: reward only every N steps (sparse)
        - Terminal state ALWAYS gets a reward (critical for final makespan)
        """
        # Check which environments are at their terminal state (per-environment check)
        # Each environment gets terminal reward when IT finishes, not when all finish
        is_terminal_per_env = (self.step_count >= self.env_number_of_ops)  # Boolean array [E]
        
        # Determine if we should give reward at this step (global check or per-env terminal)
        should_give_reward_global = (self.step_count % self.reward_step == 0)
        should_give_reward_per_env = should_give_reward_global | is_terminal_per_env  # [E]
        
        if self.reward_type == 'lb':
            current_lb = np.max(self.op_ct_lb, axis=1)
            
            if self.reward_step == 1:
                # Dense rewards: every step
                reward = self.max_endTime - current_lb
                self.max_endTime = current_lb
            else:
                # Sparse rewards: every reward_step steps OR at terminal state
                # Initialize reference on first step
                if self.reward_reference_lb is None:
                    self.reward_reference_lb = np.copy(current_lb)
                
                # Calculate reward per environment
                reward = np.where(should_give_reward_per_env,
                                self.reward_reference_lb - current_lb,  # Give reward
                                0.0)  # No reward
                
                # Update reference for environments that received reward
                self.reward_reference_lb = np.where(should_give_reward_per_env,
                                                   current_lb,
                                                   self.reward_reference_lb)
                
                # Always update max_endTime for feature computation
                self.max_endTime = current_lb
                
        elif self.reward_type == 'original':
            if self.reward_step == 1:
                # Dense rewards: every step (using normalized makespan)
                reward = self.previous_normalized_makespan - self.normalized_makespan
                self.previous_normalized_makespan = np.copy(self.normalized_makespan)
            else:
                # Sparse rewards: every reward_step steps OR at terminal state
                # Calculate reward per environment
                reward = np.where(should_give_reward_per_env,
                                self.reward_reference_normalized_makespan - self.normalized_makespan,  # Give reward
                                0.0)  # No reward
                
                # Update reference for environments that received reward
                self.reward_reference_normalized_makespan = np.where(should_give_reward_per_env,
                                                            self.normalized_makespan,
                                                            self.reward_reference_normalized_makespan)
                
                # Always update previous_normalized_makespan for tracking
                self.previous_normalized_makespan = np.copy(self.normalized_makespan)
        else:
            raise ValueError(f"Unknown reward_type: {self.reward_type}. Must be 'lb' or 'original'.")
    ...
    def step(self, actions):
        ...
        # Track normalized makespan (using normalized op_ct instead of true_op_ct)
        self.normalized_makespan[self.incomplete_env_idx] = np.maximum(
            self.normalized_makespan[self.incomplete_env_idx],
    ...
    def update_reward_step(self, new_reward_step: int):
        """
        Update reward_step for curriculum learning (dense to sparse rewards).
        
        Args:
            new_reward_step: New reward step value
        """
        if new_reward_step != self.reward_step:
            self.reward_step = new_reward_step
            # Reset reward reference points when changing reward_step
            self.reward_reference_makespan = np.copy(self.current_makespan)
            self.reward_reference_normalized_makespan = np.copy(self.normalized_makespan)
            if hasattr(self, 'op_ct_lb'):
                self.reward_reference_lb = np.max(self.op_ct_lb, axis=1)
