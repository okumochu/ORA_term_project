...
class ORSolutionStore:
    """
    Load OR (optimal/reference) makespan values from `benchmarks_data/or_solution`.
    Robust to filename prefixes like SD1_, SD2_, CL_.
    It matches by the stem present in the test instance filename (e.g., MK01, LA06, etc.).
    """
    ...
    def lookup_or_value(self, test_file_path: str) -> Optional[int]:
        ...
        3. For weakness_data files: mirror directory under benchmarks_data/or_solution
        """
        self._index_directory()

        weakness_or_file = self._weakness_or_file(test_file_path)
        if weakness_or_file:
            if os.path.exists(weakness_or_file):
                return _read_or_value_from_npy(weakness_or_file)
            return None
    ...
    def _weakness_or_file(self, test_file_path: str) -> Optional[str]:
        normalized = os.path.normpath(test_file_path)
        parts: List[str] = normalized.split(os.sep)
        if 'weakness_data' not in parts:
            return None
        idx = parts.index('weakness_data')
        rel_parts = parts[idx + 1:]
        if not rel_parts:
            return None
        rel_path = os.path.join(*rel_parts)
        stem, _ = os.path.splitext(rel_path)
        return os.path.join(self.base_dir, 'weakness_data', f'{stem}.npy')
